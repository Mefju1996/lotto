...
class LottoStatistics:
    ...
    def _read_sheet_any(self, names, **kwargs):
        last_exc = None
        for name in names:
            try:
                return pd.read_excel(self.path, sheet_name=name, engine='openpyxl', **kwargs)
...
class LottoWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.choose_stats_button = QPushButton('📊 Wczytaj plik statystyk')
        self.choose_stats_button.setStyleSheet('background:#7c3aed; color:white; padding:10px 18px; border:none; border-radius:10px; font-weight:700;')
        self.choose_stats_button.clicked.connect(self.select_stats_file)
        controls.addWidget(self.choose_stats_button)
        ...
        self.stats_label.setStyleSheet('color:#a78bfa; font-size:12px;')
    ...
    def load_stats(self, path):
        ...
        try:
            ...
            try:
                ...
                details = ''
            QMessageBox.critical(self, 'Błąd statystyk', str(exc) + details)

    def select_stats_file(self):
        start = str(self.stats_path.parent if self.stats_path else BASE_DIR)
        selected, _ = QFileDialog.getOpenFileName(self, 'Wybierz plik statystyk liczbowych', start, 'Excel (*.xlsx *.xls);;Wszystkie pliki (*)')
        if not selected:
            return
        self.load_stats(selected)
        if self.history_path is None or self.history_path == self.stats_path:
            self.history_path = Path(selected)
            self.load_history_dataframe()
        self.update_labels()
        if self.current_numbers:
            self.show_number_stats(self.current_numbers[0])
    ...
    def update_labels(self):
        stats_txt = str(self.stats_path) if self.stats_path else 'brak'
        hist_txt = str(self.history_path) if self.history_path else 'brak'
        stats_state = 'OK' if self.stats_provider else 'brak danych'
        self.stats_label.setText(f'Plik statystyk: {stats_txt} | Status: {stats_state}')
        self.path_label.setText(f'Plik historii: {hist_txt} | Arkusz: {self.history_sheet}')
    ...
    def generate_and_update(self):
        ...
        hot_rank = self.stats_provider.hot_rank if self.stats_provider else {}
        cold_rank = self.stats_provider.cold_rank if self.stats_provider else {}
        for btn, number in zip(self.ball_buttons, numbers):
            btn.set_number(number)
            if number in hot_rank:
                color = '#ef4444'
            elif number in cold_rank:
                color = '#475569'
            else:
                color = '#2563eb'
            btn.setStyleSheet(f'background:{color}; color:white; border:none; border-radius:36px; font-size:26px; font-weight:700;')
        ...
        if self.stats_provider:
            ...
            self.number_title.setText(str(numbers[0]))
            self.details_label.setText('Brak wczytanych statystyk liczbowych. Wskaż plik statystyki_lotto.xlsx.')
            self.spark.set_values([])
            self.years_label.setText('Brak danych rocznych.')
            self.pairs_label.setText('Brak danych o parach.')

    def handle_ball_click(self):
        sender = self.sender()
        if sender is None:
            return
        number = getattr(sender, 'number_value', None)
        if number is None:
            try:
                number = int(sender.text())
            except Exception:
                return
        self.show_number_stats(int(number))
